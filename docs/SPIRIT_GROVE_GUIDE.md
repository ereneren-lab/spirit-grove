...
### (재발) 가로에서 "열리는데 못 닫는 패널" — 지도가 `.overlay`가 아니었다
유저 제보: "가로 모드에서 지도 들어갔을 때, 닫을 수가 없어". 위 최적화가 **`.overlay`만 대상**이었는데
`#minimap`은 그때 `position:absolute;inset:0`짜리 **별도 패널**이라 통째로 빠져 있었다 →
세로용 캔버스(280×430)가 가로 높이(~390)를 넘겨 **닫기 버튼이 화면 밖**으로 밀리고,
`CANCELABLE_OVERLAYS`에도 없어 **Escape·B도 무효** → 리로드 말곤 못 벗어난다.
- **고친 방식은 "지도를 예외 취급"이 아니라 예외를 없애는 것**: `#minimap`을 진짜 `.overlay`로 승격
  (`.ov-head` 고정 ✕ + `.ov-body` 스크롤) → 가로 안전영역·스크롤·Escape가 **자동으로 따라온다**.
- **닫힘 뒷정리는 `closeOverlay` 한 곳에.** 지도는 rAF 루프를 도는데 그 정지가 '닫기 버튼 핸들러'에만 있어
  Escape·✕로 닫으면 루프가 살아남았다. ⚠️ `stopMinimapLoop`은 **`_miniRaf`(let) 선언 옆**에 둘 것 —
  `closeOverlay` 옆에 두면 TDZ에 걸린다(`typeof` 가드는 let에 안 통한다).
- **같이 드러난 것 — `.sheet`는 CSS 정의가 아예 없는 빈 클래스였다**(카드·구매·기술전문가·데이케어·듀오 5곳).
  본문이 길면 스크롤 없이 넘쳐 가로에서 하단 '닫기'가 화면 밖으로 나갔다 → `.sheet`를 스크롤 컨테이너로
  못박고(`flex:1;min-height:0;overflow-y:auto`), 카드엔 다른 패널과 같은 **고정 헤더 ✕**를 붙였다.
- ⚠️ **새 패널을 만들면 `CANCELABLE_OVERLAYS`에 등록할 것.** 빠지면 ✕가 가려지는 순간 탈출로가 사라진다.
- 회귀 `scripts/landscape_panel_test.js`: 패널을 **손으로 나열해 눈으로 보는 대신**, 열리는 패널마다 같은 두 단정
  (①보이는 닫기 컨트롤이 스테이지 **실좌표 안** ②Escape로도 닫힘)을 가로·세로 양쪽에서 건다.
  ⚠️ 크기 0 단정을 같이 둬야 "안 열려서 rect가 0이라 통과"하는 거짓 양성을 막는다.
...
- **(대형) `findTile`의 영구 null 캐시가 목표 사슬에서 결말 두 개를 지웠다**: `findTile`은 결과를 캐시하는데
  `MAP_STR`·`W`·`H`는 **실내에 들어가면 인테리어 지도로 통째로 바뀐다.** 그리고 `currentGoal()`은 뱃지<4면
  **체육관 단계에서 리턴**하므로 `%`(호수 제단)·`X`(군주의 제단)는 그전까지 **한 번도 조회되지 않는다** →
  두 문자의 **최초 스캔이 4번째 뱃지를 따는 순간, 즉 항상 체육관 안**에서 일어나 `null`이 세션 내내 캐시됐다.
  `if(alt)`·`if(lord)` 가드가 조용히 통과해 **4뱃지 → 곧장 리그**가 되고, 전설 준비 단계와 숲의 군주 단계가
  (즉 **플롯 결말 EPILOGUE가**) 사슬에서 통째로 빠졌다. 어제 넣은 두 기능이 실플레이에서 무효였다.
  → 실내면 **`_owBackup`(오버월드 원본)** 을 스캔 + **null은 캐시하지 않는다**(스캔 25×49 = 비용 없음).
  - ⚠️ **리로드하면 캐시가 비어 증상이 사라진다** → 손으로는 재현이 거의 안 되고, 런타임 에러도 안 난다.
  - ⚠️ **`lord_route_test`가 구조적으로 못 잡는 테스트였다** — 상태를 **야외에서** 조립하고 트래커를 처음 읽어
    캐시가 올바른 값으로 채워졌다. 초록이라 "경로는 확정"이라 적었는데 틀렸다. → **[7]** 구간 신설:
    **실내에서 4뱃지가 되는 순간**을 재현한다(수정 전 빌드에서 실패 확인).
  - ⚠️ **교훈**: 픽스처가 **상태를 어디서 조립하느냐**가 커버리지를 바꾼다. **순서 의존 캐시는 순서를 재현해야** 잡힌다.
    "실내/실외에서 재야 할 것을 반대로 잰다"는 이 저장소가 이미 여러 번 밟은 형태다(`lord_route_test` [4][6] 주석).
...
### 지선 루트 (통과형 · 현재 6개) — `ROUTE_AT` 좌표 스코프
오버월드가 25×50 **세로 복도 하나**라 일차원적이었다 → **입구가 둘인 통과형 인테리어**로 지도에 순환을 만든다.
현재 6개이고 **모든 지역 경계에 하나씩** 있다: 언덕길(1↔2) · 갱도(2↔3) · **이끼 골짜기(3↔4)** ·
잔교(4 내부) · **물안개 벼랑길(4↔5)** · 고갯길(5↔6). 초원부터 리그까지 지선만 타고 이어진다.
- 타일 문자는 `$` **하나**를 쓰고 `ROUTE_AT`이 좌표로 스코프한다(`GYM_AT`와 같은 방식 — 문자 하나로 무한 확장).
- **게이팅 불변이 대전제**: 두 입구 모두 원래 걸어서 갈 수 있던 곳이어야 한다(회귀가 강제). 지름길만 생긴다.

**새 루트를 팔 때(`scripts/route_probe.js`로 좌표부터 뽑을 것 — 감으로 찍지 말 것):**
1. 입구는 **벽(`#`) 타일**에 놓는다 → `$`도 비보행이라 **통행성이 안 변한다**(연결성 검증 불필요).
2. 실제 `walkable()` BFS로 (a)착지 칸(입구 바로 아래)이 도달 가능하고 (b)**두 입구의 도보 거리가 먼** 쌍을 고른다.
   ⚠️ 가까운 쌍을 고르면 루트를 팔 이유가 없다 — 후보쌍이 5칸부터 50칸까지 갈렸다(골짜기 21 · 벼랑길 25를 건너뛴다).
   ⚠️ **착지 칸의 지역 밴드**로 판단할 것 — 입구가 밴드 경계 행에 있으면 착지가 옆 지역이라 의도와 달라진다.
3. 배선 9곳: `MAP_STR`의 `$` · `ROUTE_AT`(sx/sy) · `INTERIORS`(exits) · `ENC_POOLS` · `start*Encounter` ·
   `wildRoll` 분기 · `fieldMusic` · `HABITAT_KO` · `GROUND_ITEMS`(`in:`) · 야외 렌더 목록(아래).
4. ⚠️ **시작 칸(sx,sy)이 출구 칸과 같으면 안 된다** — 들어가자마자 출구를 밟아 튕겨 나온다(`route_test [1-b]`).

**이번에 밟은 함정 3건:**
- ⚠️ **야외 스타일 목록에 안 넣으면 동굴/모래색 폴백**으로 떨어진다(주석이 경고하고 있었는데도 밟았다).
  `_tile`의 야외형 인테리어 분기에 **id 3곳**(낮/밤 틴트 목록 · 분기 조건 · 지역 플래그)과 **바닥색**을 같이 넣을 것.
- ⚠️ 그 분기는 **끝에서 `return`** 한다 → 거기 없는 타일은 바닥색만 칠해진다. 모닥불(`R`)이 안 그려져
  "밟으면 회복되는 빈 칸"이 됐다. 새 타일을 쓰면 그 분기 안에도 렌더를 넣을 것.
- ⚠️ **`R`(모닥불)은 원래 `onArrived`의 실외 블록에만 있었다** — 실내 블록이 먼저 return하므로 인테리어에 놓아도
  장식이었다. 실내 분기를 추가해야 실제로 회복된다(이끼 골짜기 야영지 = 루트 최초의 쉼터).
  `markHeal`은 실내면 `_owBackup.pos`(들어온 오버월드 입구)를 기록하므로 부활 지점은 안전하다.
- 회귀 `scripts/route_test.js`(전 루트 순회 — 새 루트 자동 커버) + `indoor_encounter_test`에 **루트 6개 포함**
  (⚠️ 예전엔 특수 지역만 보고 **루트가 통째로 빠져 있었다** — 같은 실내 블록을 지나므로 같이 죽을 수 있다).
...
### 게임 디자인 평가 후속 7건 (2026-07-29) — 되돌리기 전에 근거를 볼 것
데이터 실측으로 갭을 찾아 순서대로 처리했다. **각 항목의 '왜'가 커밋 메시지에 있다.**
1. **전설 준비 단계**(트래커): 4뱃지 → 곧장 리그였다 → 전설 미보유면 호수 제단을 끼운다.
   리그 완주율이 전설 보유로 갈리는데(Lv48 7.6%→18.4%) 그 의도가 전달되지 않았다.
2. **땅·독 로스터 복구**: 1차 타입 ground 1종·poison 0종이었다 → 9종 재타이핑(ground 3→8 · poison 2→6).
   ⚠️ 땅은 상성표에서 유리 4개로 가장 강한 공격 타입인데 쓸 정령이 없었다 = 전략 공간 20%가 죽어 있었다.
3. **타입별 공격기 ≥6**: 불 14종이 나눠 쓸 불 기술이 4개였다 → 13종 추가.
   ⚠️ **전부 타입 전용 TM** — 학습셋에 안 넣어 트레이너·야생이 못 쓴다 = 밸런스 불변.
4. **학습셋 확장** 중앙값 4 → 8(총 387→780). 코드젠 `scripts/expand_learnsets.py`(재현 가능).
   ⚠️ **자연 학습 풀 71종만** 후보로 쓴다 — TM 전용 37종이 학습셋에 새어들면 "플레이어 전용" 경계가 깨진다.
5. **tier3 종족값 압축** 141~224 → 175~205(폭 83→30). 상한을 205로 둔 건 224/210/205가 미니보스라서다.
6. **초반 밀도**: 반딧불 오솔길(7번째 지선, 마을↔초원 25칸) + 초반 퀘스트 2종. 이탈 구간이 가장 비어 있었다.
7. **진화 라인 8개 추가**: 독립종 25종 중 **모티프가 실제로 이어지는 쌍만** 묶었다(아트 0장).
   진화 라인 소속 55→72/86(84%). ⚠️ 억지 연결은 「아트/설계 규칙」의 "같은 색·모티프" 위반이라
   10~15개가 아니라 8개에서 멈췄다. 새 종을 만들려면 아트가 먼저다.
- ⚠️ **2·4·5·7은 트레이너·야생의 강함을 바꾼다** → 각 단계마다 `balance_test`·`league_test`를 재측정했다.
  한꺼번에 몰아서 재면 무엇이 원인인지 못 가린다.
