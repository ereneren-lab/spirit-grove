...
- ⚠️ 연출 테스트는 **스크린샷 타이밍에 의존하지 말고** MutationObserver로 배틀필드에 추가된 DOM의 지문을 볼 것. 그리고 `el.style.cssText`는 **콜론 뒤에 공백을 넣어 직렬화**되므로(`width: 100%`) 스타일 정규식에 `\s*`가 필요하다. 낙뢰 기둥처럼 `linear-gradient`를 쓰는 형태가 여럿이면 판정 순서도 주의.
