...
### ⚠️ 밸런스 하네스는 상태이상을 모델링하지 않는다
`balance_test.js` / `league_test.js`의 `applyMove`는 `power>0` · `heal` · `eff.stat`만 처리한다 — **`eff.status`는 아예 없다**(둘 다 0건). 그래서:
- 상태이상 관련 변경(타입 면역·맹독·잠듦 등)을 넣고 이 테스트가 "그대로"라고 나와도 **안전성이 증명된 게 아니다.** 애초에 측정 대상이 아니었을 뿐이다.
- 난이도 커브 자체가 상태이상을 뺀 채 잡혀 있다는 뜻이기도 하다. 상태이상을 실제로 재려면 하네스에 `applyStatus` 경로를 추가해야 한다(미구현).
...
`playthrough_test.js:146`은 전투 후 `busy`가 풀리기를 **15초(60×250ms) 고정 예산**으로 기다린다. load 30~50에선 승리 연출·경험치 애니가 그 안에 안 끝나 `G.busy가 풀려 있다 (true)`로 실패한다 — **게임 버그가 아니다**.
