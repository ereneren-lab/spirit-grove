...
setTimeout(() => {
  ...
  // ⚠️ 이 wildLevel 모델은 레벨만 본다 — 실전투 승률은 balance_test[초반야생]이 __SIM으로 잰다.
  //    (하한 7=21.6% → 6=29.7%로 개선. 옛 주석의 "41%"는 이 XP모델 추정이었다.)
  const region1Wild = wildLevel(5, 44);   // 지역1(y=44), 파티 평균 = 스타터 Lv5
  // ⚠️ 임계값 +2: 하한 6으로 낮춰 야생 Lv6(+1)이 됐다. 진화형 매복은 [6]이 따로 못박는다.
  ok(region1Wild - 5 <= 2, `지역1 첫 야생 Lv${region1Wild} — 스타터 Lv5 대비 +${region1Wild-5} (≤+2, 벽 방지)`);
