...
(async()=>{
  ...
  // ⚠️ 전투 횟수는 봇의 무작위 동선·전멸(시작 지역 저조우대로 리셋)에 크게 흔들린다 → 하드 게이트로
  //    두면 간헐 실패한다(문서화된 안티패턴). "아예 못 싸운다(0=고장)"만 게이트하고 수치는 참고 출력.
  ok(stats.battles>=1, `전투가 실제로 발생했다 (${stats.battles}회, 예산 짧으면 적다)`);
  ok(fin.party>=1, `파티가 유지된다 (${fin.party}마리)`);
  // ⚠️ 누적 XP는 '봇이 이긴 전투 수'에 달렸다 — 무작위 플레이라 짧은 예산에선 전부 지고 0일 수도(전멸).
  //    XP 지급 메커니즘은 exp_test/winBattle이 검증하므로, 여기선 난이도 참고치로만 출력(하드 게이트 X).
  console.log(`  ℹ️  누적 XP +${stats.xpGained||0} (봇이 이긴 전투에 달림 — 경향치)`);
