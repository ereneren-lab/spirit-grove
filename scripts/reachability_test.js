...
(async()=>{
  ...
  const R=await p.evaluate(async()=>{
    ...
    for(let y=0;y<50;y++)for(let x=0;x<25;x++){ const t=F.tileAt(x,y);
      ...
      if(t==="V")all.push("lavacave"); if(t==="M")all.push("snowfield"); if(t==="J")all.push("marsh"); if(t==="W")all.push("coast"); }
  ...
  const IN=await p.evaluate(async()=>{
    ...
    for(const id in F.INTERIORS){
      ...
      G.party=[S.makeMon("foxfire",30)];
      // 모든 가드를 격파한 상태 = 최대 개방
      "0123456789abcdefijklqruvXANS".split("").forEach(c=>G.defeated.add(c));
      F.enterInterior(I);
