...
// ⚠️ **위 기록은 숲의 군주가 트래커 사슬 밖이던 시절 것이다.** 2026-08-04에 사슬을 바꿨다:
//    `4뱃지 → (전설) → 숲의 군주 → 정령 리그`. 이제 봇도 제단을 거쳐 간다(그전엔 안 갔다).
//    → 위 완주 기록(체육관4 1868초 → 챔피언 2952초)에는 **군주전이 빠져 있다**. 다음 완주 때 갱신할 것.
//    경로 자체(트래커 안내 → 정면 문 → 가드 → 제단 A → 보스 시작 → 리그로 전환)는
//    `scripts/lord_route_test.js`가 실입력으로 검증한다(부하 높은 환경에서도 몇 분이면 돈다).
//
