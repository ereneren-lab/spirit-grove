# 정령 픽셀 프롬프트 — 남은 74종 (붙여넣기용)

> Phase 1 완료분 12종(스타터 3라인+야생 3종)은 제외. 각 블록 **Prompt 전체를 그대로 복사**해 생성(투명배경).
> 파일명=정령 id.png로 GitHub 웹에서 `art_inbox/creatures_pixel/`에 업로드 → Claude가 96px 변환.
> ⚠️ 이름 변경·진화 재설계 반영된 최신본. Negative는 별도 필드에.
