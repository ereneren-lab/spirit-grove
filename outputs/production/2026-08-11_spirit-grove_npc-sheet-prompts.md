...
작성 2026-08-11 · **✅ 1차 12 + 2차 5 + 3차-A 8 + 3차-B 7 = 시트 32종 투입 완료**
...
## 8-B. 발주 B — **겹침 해소 변형 7장** — ✅ **2026-08-11 도착·투입 완료**

> 📌 **레이아웃이 뒤집혀 왔다** — 7명 × 3자세가 **7열 × 3행**(열=인물, 행=자세)이었다.
> 같은 프롬프트인데 1·2·3-A차와 다르게 깔렸다. 다시 요구하지 않고 `--layout cols`를 추가했다.
> **생성기는 레이아웃을 보장하지 않는다 — 읽는 쪽에서 받아준다.**
> 결과: 겹침 **14쌍 → 3쌍**(라이벌 카이 2쌍 제외하면 **1쌍**. 호수지기 리오·낚시꾼 도윤, 5칸 거리).
> ⚠️ 이때 `NPC_ARCH`에 **중복 키 사고**가 났다 — 아래에 남은 옛 줄이 새 배정을 조용히 덮었다.
>    `npc_sheet_test [7]`이 이제 소스 텍스트를 훑어 잡는다.
