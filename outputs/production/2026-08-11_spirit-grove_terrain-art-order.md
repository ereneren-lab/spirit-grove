...
작성 2026-08-11 · **✅ 건물 7 + 오브젝트 5 = 12종 투입 완료** (`tallgrass`만 보류 — §7-4)

> **유저가 후보 2벌 중 2번을 골랐다.** 1번은 물체 밑에 **잔디가 그려져 있어** 고원·제단에서도
> 초록 패치가 따라다녔을 것이다(§3의 "바닥 그리지 말 것"이 막으려던 게 정확히 이것).
> 채택 안 된 1번은 `art_inbox/terrain/_rejected/`에 남겨 뒀다(git에서는 제외).
...
## 7-4. ✅ 투입 결과 (2026-08-11)

**지형 아트 12종** — 건물 7(center·shop·hall·gym·dojo·home·altar) + 오브젝트 5(tree_s·rock·sign·campfire·tree_l).

### 지역 틴트를 넣었다 (§5-2의 선택지 중 **2번**)
`terrainTinted()`가 나무·풀에 `REGIONPAL` 색을 곱연산으로 얹고 **(아트 × 지역) 조합마다 캐시**한다.
실측: `tree_s` 마을 rgb(44,68,30) → 고원 rgb(56,65,30) · `tree_l` rgb(43,56,35) → rgb(56,56,36).
📌 NPC 때 틴트를 기각한 이유가 "받은 그림이 이미 원색"이었는데, **이번엔 무채색으로 받아 조건이 맞았다.**
⚠️ 눈으로만 보면 다음에 조용히 깨진다 → `terrain_art_test [5]`가 **픽셀을 직접 읽어** 단정한다.

### ⚠️ 아트로 대체하면 **그 가지 안의 애니메이션도 같이 사라진다** — 하루에 두 번 겪었다
1. **보스 오라**(`isleLeader`·`snowMaster`) — 시트 분기가 그림을 그리고 바로 `return`했다.
2. **모닥불 불꽃**(`R`) — 지형 훅이 `t==="R"` 가지를 통째로 건너뛰어 **차가운 돌무더기**가 됐다.
   모닥불은 회복 지점이고 불꽃이 그 신호다.
→ 둘 다 **두 경로가 같은 함수를 쓰도록** 뺐다(`_bossAura` · `_fireFx`).
**다음에 절차적 그림을 아트로 바꿀 땐 그 가지 안에 `now`를 쓰는 코드가 있는지 먼저 볼 것.**

### ⏸ `tallgrass`(타일 `T`)는 보류
변환은 해뒀지만 **매니페스트에 넣지 않았다.** `T`는 오토타일이라 이웃과 변을 이어 붙여
하나의 덩어리로 그린다(07-28에 "맵이 너무 타일처럼"을 고치려고 넣은 장치다).
한 장짜리 이미지로 바꾸면 그 이음이 사라져 격자가 다시 드러난다. 시험해 보고 싶으면
`TERRAIN_ART_ID`에 `T:"tallgrass"`를 넣고 매니페스트에 추가하면 된다 — 되돌리기도 그만큼 쉽다.
