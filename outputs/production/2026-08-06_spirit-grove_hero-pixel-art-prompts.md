...
## 2. ⚠️ 먼저 고쳐야 할 코드 2가지 (안 하면 프롬프트가 아무리 좋아도 뭉개진다)

**① 캔버스 스무딩이 켜져 있다.** `imageSmoothingEnabled`가 **소스 어디에도 없다** = 브라우저 기본값 `true`.
픽셀 아트를 이대로 넣으면 보간돼서 흐릿해진다. 필드 캔버스 컨텍스트에 `ctx.imageSmoothingEnabled=false`가 필요하다.

**② 주인공 그리기 크기가 정수배가 아니다.** 실측:
...
32×32 원본을 75px로 늘리면 2.34배라 **어떤 픽셀은 2칸, 어떤 픽셀은 3칸**이 되어 도트가 울퉁불퉁해진다.
`S`를 32의 정수배(64 또는 96) 중 가까운 값으로 스냅하면 도트가 딱 떨어진다.

> 둘 다 두어 줄짜리 수정이다. 아트를 받기 전에 먼저 넣어두는 게 맞다 — 말만 하면 바로 하겠다.
...
- **미확정**: 도트 크기 스냅(§2-②)을 넣으면 주인공이 지금보다 살짝 작아진다(75 → 64px).
  타일보다 작아지는 게 GBA풍에 더 맞지만, 커 보이길 원하면 96px로 올릴 수도 있다. **선택 필요.**
- **대안(아트 0장)**: `HERO_ART`를 빼면 코드가 이미 가진 절차적 워커(`_walker`)로 되돌아간다.
  NPC와 완전히 같은 방식이라 이질감이 **즉시** 사라진다. 품질은 지금 아트보다 소박하다.
  픽셀 아트를 만들기 전에 이걸 한 번 보고 비교하는 게 싸다.
